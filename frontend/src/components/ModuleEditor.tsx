...
export default function ModuleEditor({ moduleId, onClose }: ModuleEditorProps) {
  const { modules, createNewModule, updateModuleData, deleteStepData, reorderSteps } = useModuleStore();
  ...
  const handleDeleteStep = async (stepId: string) => {
    if (!module || !confirm('Are you sure you want to delete this step?')) return;
                  ...
                  {module.steps.map((step, index) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="text-sm font-medium text-gray-900 truncate">{step.title}</h4>
                        <p className="text-xs text-gray-500 capitalize">{step.type === 'freeResponse' ? 'Free Response' : step.type}</p>
                      </div>
